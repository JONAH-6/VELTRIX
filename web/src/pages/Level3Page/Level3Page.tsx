...
const Level3Page = () => {
  ...
  const [health, setHealth] = useState(5) // starting health (unchanged)
  const [timeLeft, setTimeLeft] = useState(30) // time limit remains 30s
  ...
  // HARDER parameters for Level 3
  const WIN_SCORE = 300 // was 250 in Level 2
  const TARGET_DISPLAY_TIME = 600 // milliseconds (was 700) – even faster
  const PENALTY_MISS = 2 // still lose 2 health (could increase to 3, but keep 2 for now)
  ...
  const showTarget = () => {
    if (!gameActive) return
    const container = document.getElementById('game-container-level3')
    if (!container) return
    ...
    timeoutRef.current = setTimeout(() => {
      if (targetVisible && gameActive) {
        ...
        setTimeout(() => showTarget(), 350) // shorter delay after miss
      }
    }, TARGET_DISPLAY_TIME)
  }
  ...
  const handleTargetClick = () => {
    ...
    setTimeout(() => showTarget(), 180) // shorter delay after success
  }
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-to-br from-gray-900 to-gray-800">
      ...
      <div className="bg-gray-800 p-3 text-center text-gray-300 text-sm border-t border-gray-700">
        Extreme: Target disappears in 0.6s! Reach {WIN_SCORE} points to win.
      </div>
    </div>
  )
}
